...
/**
 * A command that calls the moveToFoundation method on a KlondikeModel.
 */
public class MPFCommand implements KlondikeCommand {
  private final Scanner scanner;

  /**
   * A constructor for MPFCommand.
   * @param scanner A scanner used for reading input
   */
  public MPFCommand(Scanner scanner) {
  ...
  public ControllerStatus run(KlondikeModel model, TextualView view, Appendable out) {
    ...
    ArrayList<Integer> args = KlondikeIOUtils.getIntsOrQuit(2, this.scanner, out);
    if (args.size() != 2) { // if all inputs weren't collected, then input must have quit
